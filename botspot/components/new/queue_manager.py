...
class Queue(Generic[T]):
    ...
    async def get_next_item(self, mark_done: bool = True) -> Optional[T]:
        ...
        item_dict = await collection.find_one(
            query, sort=[("priority", -1), ("added_at", 1)]
        )
        ...
        if mark_done:
            await collection.update_one(
                {"_id": item_dict["_id"]}, {"$set": {"is_done": True}}
            )
    ...
    async def get_random_item(self, mark_done: bool = True) -> Optional[T]:
        ...
        item_dict = (
            await collection.find(query).skip(random_index).limit(1).to_list(length=1)
        )
...
class QueueManager:
    ...
    def get_queue(
        self, key: str = "default", item_model: Type[T] = QueueItem
    ) -> Queue[T]:
        """Get a queue by key."""
...
def initialize(settings: QueueManagerSettings) -> QueueManager:
    ...
    if settings.commands_visible:
        try:
            ...
            add_command(
                "queue_show", "Show all items in your queue", Visibility.PUBLIC
            )(show_queue_handler)
            add_command(
                "queue_clear", "Clear all items from your queue", Visibility.PUBLIC
            )(clear_queue_handler)
        except ImportError:
            logger.warning(
